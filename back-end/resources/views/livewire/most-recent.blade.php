    ...
    <div class="products-section-row">
        @foreach ($recentProduct as $product)
            @if ($product->inventory->quantity < 1 || $product->status != 1)
                <div class="product"> <a href="{{ route('product.detail', $product->id) }}">
                    ...
                    <form wire:submit.prevent="addItemToCart({{ $product->id }})">
                        ...
                        <button type="button" disabled style="background: gray" type="submit"
                            class="add-cart addToCart">Add to cart</button>
                    </form>
                ...
                <div class="product">
                  <a href="{{ route('product.detail', $product->id) }}">
                        ...
                        <div class="productInfo">
                            <p>{{ $product->brand->name }}</p>
                            <p><strong> {{ $product->productName }} </strong></p>
                            <span class="price"> <del>{{ $product->productCost }}$ </del>
                                {{ $product->price }}$</span>
